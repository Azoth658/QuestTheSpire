...
public class RemoveAllBuffsAction extends AbstractGameAction {
    ...
    public RemoveAllBuffsAction(AbstractCreature target, AbstractGameEffect effect) {
        this(target, effect, null, 0);
    }

    public RemoveAllBuffsAction(AbstractCreature target, String sfxKey, float pitchVariance) {
        this(target, null, sfxKey, pitchVariance);
    }

    public RemoveAllBuffsAction(AbstractCreature target, AbstractGameEffect effect, String sfxKey, float pitchVariance) {
        this.target = target;
        vfx = effect;
        key = sfxKey;
    ...
    public void update() {
        ...
        for (AbstractPower p : target.powers) {
            if (p.type == AbstractPower.PowerType.BUFF && !(p instanceof InvisiblePower)) {
